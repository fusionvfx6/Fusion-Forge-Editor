...
class ProtocolBuffer {
	...
	public append(data: string | Buffer): void {
		...
		if (this.buffer.length - this.index >= toAppend.length) {
			toAppend.copy(this.buffer as Uint8Array<ArrayBufferLike>, this.index, 0, toAppend.length);
		} else {
			const newSize = (Math.ceil((this.index + toAppend.length) / defaultSize) + 1) * defaultSize;
			if (this.index === 0) {
				this.buffer = Buffer.allocUnsafe(newSize);
				toAppend.copy(this.buffer as Uint8Array<ArrayBufferLike>, 0, 0, toAppend.length);
			} else {
				this.buffer = Buffer.concat([this.buffer.slice(0, this.index) as Uint8Array<ArrayBufferLike>, toAppend as Uint8Array<ArrayBufferLike>], newSize);
			}
		}
	...
	public tryReadContent(length: number): string | null {
		...
		this.buffer.copy(this.buffer as Uint8Array<ArrayBufferLike>, 0, sourceStart);
