...
MenuRegistry.appendMenuItems([{
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar left', "Move FusionForge Side Bar Left")
		},
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar right', "Move FusionForge Side Bar Right")
		},
			...
			containerModel.visibleViewDescriptors.forEach(viewDescriptor => {
				if (viewDescriptor.canMoveView) {
					if (!hasAddedView) {
						results.push({
							type: 'separator',
							label: localize('secondarySideBarContainer', "FusionForge Side Bar / {0}", containerModel.title)
						});
						hasAddedView = true;
					}
	...
	run(accessor: ServicesAccessor, viewId?: string): void {
		...
		if (!(isViewSolo && currentLocation === ViewContainerLocation.AuxiliaryBar)) {
			items.push({
				...
				label: localize('moveFocusedView.newContainerInSidePanel', "New FusionForge Side Bar Entry")
			});
		}
		...
		items.push({
			type: 'separator',
			label: localize('secondarySideBar', "FusionForge Side Bar")
		});
...
ToggleVisibilityActions.push(...[
	...
	CreateToggleLayoutItem(ToggleAuxiliaryBarAction.ID, AuxiliaryBarVisibleContext, localize('secondarySideBar', "FusionForge Side Bar"), { whenA: ContextKeyExpr.equals('config.workbench.sideBar.location', 'left'), iconA: panelRightIcon, iconB: panelLeftIcon }),
