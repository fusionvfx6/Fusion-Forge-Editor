...
// The extension this was called from is here - https://github.com/fusionvfx6/Fusion-Forge-Editor
...
export class AutocompleteService extends Disposable implements IAutocompleteService {
	...
	static readonly ID = 'fusionforge.autocompleteService'
		...
		newAutocompletion.llmPromise = new Promise((resolve, reject) => {
			...
			const requestId = this._llmMessageService.sendLLMMessage({
				...
				onText: ({ fullText }) => {
					...
					// count total newlines in fullText
					const numNewlines = fullText.match(/\n|\r\n/g)?.length || 0

					// if too many newlines, abort early to avoid waiting for 300 tokens
					if (numNewlines > 5) {
