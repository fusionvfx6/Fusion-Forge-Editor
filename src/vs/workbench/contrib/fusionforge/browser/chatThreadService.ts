...
import { FusionForgeFileSnapshot } from '../common/editCodeServiceTypes.js';
...
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart FusionForge)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart FusionForge), cancel that last tool
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		...
		const userModifiedFusionForgeFileSnapshot = fsPath in checkpointMessage.userModifications.voidFileSnapshotOfURI ? checkpointMessage.userModifications.voidFileSnapshotOfURI[fsPath] ?? null : null
		return { voidFileSnapshot: userModifiedFusionForgeFileSnapshot ?? voidFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const voidFileSnapshotOfURI: { [fsPath: string]: FusionForgeFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			...
			if (!res) continue
			const { voidFileSnapshot: oldFusionForgeFileSnapshot } = res
			...
			const voidFileSnapshot = this._editCodeService.getFusionForgeFileSnapshot(URI.file(fsPath))
			if (oldFusionForgeFileSnapshot === voidFileSnapshot) continue
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getFusionForgeFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					this._editCodeService.restoreFusionForgeFileSnapshot(URI.file(fsPath), voidFileSnapshot)
					break
				}
			}
		}
