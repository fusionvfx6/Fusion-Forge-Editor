...
import { IFusionForgeCommandBarService } from './voidCommandBarService.js';
...
import { IFusionForgeSettingsService } from '../common/voidSettingsService.js';
import { FeatureName } from '../common/voidSettingsTypes.js';
import { IFusionForgeModelService } from '../common/voidModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, FusionForgeFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@IFusionForgeSettingsService private readonly _settingsService: IFusionForgeSettingsService,
		// @IFileService private readonly _fileService: IFileService,
		@IFusionForgeModelService private readonly _voidModelService: IFusionForgeModelService,
	...
	// 		message: `FusionForge Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'fusionforge.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open FusionForge's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(VOID_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/fusionvfx6/Fusion-Forge-Editor/issues/new) it.` : undefined
	// 	})
	// }
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'fusionforge-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'fusionforge-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'fusionforge-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'fusionforge-greenBG', {
		...
		if (type !== 'insertion') {
			const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					const domNode = document.createElement('div');
					domNode.className = 'fusionforge-redBG'
		...
		if (diffZone.type === 'DiffZone' && !diffZone._streamState.isStreaming) {
			...
			const consistentWidgetId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					else { throw new Error('FusionForge 1') }
	...
	private _getCurrentFusionForgeFileSnapshot = (uri: URI): FusionForgeFileSnapshot => {
		const { model } = this._voidModelService.getModel(uri)
	...
	private _restoreFusionForgeFileSnapshot = async (uri: URI, snapshot: FusionForgeFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: FusionForgeFileSnapshot = this._getCurrentFusionForgeFileSnapshot(uri)
		let afterSnapshot: FusionForgeFileSnapshot | null = null

		const elt: IUndoRedoElement = {
			type: UndoRedoElementType.Resource,
			resource: uri,
			label: 'FusionForge Agent',
			code: 'undoredo.editCode',
			undo: async () => { opts?.onWillUndo?.(); await this._restoreFusionForgeFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreFusionForgeFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentFusionForgeFileSnapshot(uri)
	...
	public getFusionForgeFileSnapshot(uri: URI) {
		return this._getCurrentFusionForgeFileSnapshot(uri)
	}


	public restoreFusionForgeFileSnapshot(uri: URI, snapshot: FusionForgeFileSnapshot): void {
		this._restoreFusionForgeFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`FusionForge: diff.type not recognized on: ${lastDiff}`)
		}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`FusionForge: diff.type not recognized on: ${from}`)
		}
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`FusionForge: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	// called on fusionforge.acceptDiff
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`FusionForge error: ${diff}.type not recognized`)
		}
	...
	// called on fusionforge.rejectDiff
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IFusionForgeCommandBarService private readonly _voidCommandBarService: IFusionForgeCommandBarService,
