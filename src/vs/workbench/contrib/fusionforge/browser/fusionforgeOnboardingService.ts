...
import { mountFusionForgeOnboarding } from './react/out/fusionforge-onboarding/index.js'
...
export class OnboardingContribution extends Disposable implements IWorkbenchContribution {
	...
	private initialize(): void {
		...
		if (workbench) {
			...
			const onboardingContainer = h('div.fusionforge-onboarding-container').root;
			workbench.appendChild(onboardingContainer);
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountFusionForgeOnboarding(onboardingContainer, accessor);
