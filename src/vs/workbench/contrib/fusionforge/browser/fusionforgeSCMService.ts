...
import { IFusionForgeSCMService } from '../common/voidSCMTypes.js'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private voidSCM: IFusionForgeSCMService
	private loadingContextKey: IContextKey<boolean>

	constructor(
		@ISCMService private readonly scmService: ISCMService,
		@IMainProcessService mainProcessService: IMainProcessService,
		@IFusionForgeSettingsService private readonly voidSettingsService: IFusionForgeSettingsService,
		...
		this.voidSCM = ProxyChannel.toService<IFusionForgeSCMService>(mainProcessService.getChannel('fusionforge-channel-scm'))
	}
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'FusionForgeSCM - Commit Message' },
			})
		})
	}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'fusionforge.generateCommitMessageAction',
			title: localize2('voidCommitMessagePrompt', 'FusionForge: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('voidCommitMessagePromptTooltip', 'FusionForge: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'fusionforge.loadingGenerateCommitMessageAction',
			title: localize2('voidCommitMessagePromptCancel', 'FusionForge: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('voidCommitMessagePromptCancelTooltip', 'FusionForge: Cancel Commit Message Generation'),
