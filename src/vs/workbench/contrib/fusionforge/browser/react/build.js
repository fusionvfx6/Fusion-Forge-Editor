...
function saveStylesFile() {
	setTimeout(() => {
		try {
			const pathToCssFile = findDesiredPathFromLocalPath('./src/vs/workbench/contrib/fusionforge/browser/react/src2/styles.css', __dirname);
...
if (isWatch) {
	...
	if (!fs.existsSync('src2')) {
		try {
			...
			execSync(
				'npx scope-tailwind ./src -o src2/ -s fusionforge-scope -c styles.css -p "fusionforge-"',
				{ stdio: 'inherit' }
			);
	...
	execSync('npx scope-tailwind ./src -o src2/ -s fusionforge-scope -c styles.css -p "fusionforge-"', { stdio: 'inherit' });
