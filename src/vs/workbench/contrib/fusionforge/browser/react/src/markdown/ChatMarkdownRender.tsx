...
const Codespan = ({ text, className, onClick, tooltip }: { text: string, className?: string, onClick?: () => void, tooltip?: string }) => {
	...
	return <code
		className={`font-mono font-medium rounded-sm bg-fusionforge-bg-1 px-1 ${className}`}
		onClick={onClick}
		{...tooltip ? {
...
const RenderToken = ({ token, inPTag, codeURI, chatMessageLocation, tokenIdx, ...options }: { token: Token | string, inPTag?: boolean, codeURI?: URI, chatMessageLocation?: ChatMessageLocation, tokenIdx: string, } & RenderTokenOptions): React.ReactNode => {
	...
	if (t.type === 'table') {
		...
		// return (
		// 	<div>
		// 		<table className={'min-w-full border border-fusionforge-bg-2'}>
		// 			<thead>
		// 				<tr className='bg-fusionforge-bg-1'>
		// 					{t.header.map((cell: any, index: number) => (
		// 						<th
		// 							key={index}
		// 							className='px-4 py-2 border border-fusionforge-bg-2 font-semibold'
		...
		// 					<tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-fusionforge-bg-1'}>
		// 						{row.map((cell: any, cellIndex: number) => (
		// 							<td
		// 								key={cellIndex}
		// 								className={'px-4 py-2 border border-fusionforge-bg-2'}
	...
	if (t.type === 'link') {
		return (
			<a
				...
				className='underline cursor-pointer hover:brightness-90 transition-all duration-200 text-fusionforge-fg-2'
			>
				{t.text}
			</a>
		)
