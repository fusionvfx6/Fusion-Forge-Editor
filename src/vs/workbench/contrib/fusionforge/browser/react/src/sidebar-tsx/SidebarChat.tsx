...
import { BlockCode, TextAreaFns, FusionForgeCustomDropdownBox, FusionForgeInputBox2, FusionForgeSlider, FusionForgeSwitch, FusionForgeDiffEditor } from '../util/inputs.js';
import { ModelDropdown, } from '../fusionforge-settings-tsx/ModelDropdown.js';
import { PastThreadsList } from './SidebarThreadSelector.js';
import { VOID_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { VOID_OPEN_SETTINGS_ACTION_ID } from '../../../voidSettingsPane.js';
import { ChatMode, displayInfoOfProviderName, FeatureName, isFeatureNameDisabled } from '../../../../../../../workbench/contrib/fusionforge/common/voidSettingsTypes.js';
import { ICommandService } from '../../../../../../../platform/commands/common/commands.js';
import { WarningBox } from '../fusionforge-settings-tsx/WarningBox.js';
...
import { ToolApprovalTypeSwitch } from '../fusionforge-settings-tsx/Settings.js';
...
const ReasoningOptionSlider = ({ featureName }: { featureName: FeatureName }) => {
	...
	if (canTurnOffReasoning && !reasoningBudgetSlider) { // if it's just a on/off toggle without a power slider
		return <div className='flex items-center gap-x-2'>
			<span className='text-fusionforge-fg-3 text-xs pointer-events-none inline-block w-10 pr-1'>Thinking</span>
			<FusionForgeSwitch
				size='xxs'
	...
	if (reasoningBudgetSlider?.type === 'budget_slider') { // if it's a slider
		...
		return <div className='flex items-center gap-x-2'>
			...
			/>
			<span className='text-fusionforge-fg-3 text-xs pointer-events-none'>{isReasoningEnabled ? `${value} tokens` : 'Thinking disabled'}</span>
		</div>
	}
	...
	if (reasoningBudgetSlider?.type === 'effort_slider') {
		...
		return <div className='flex items-center gap-x-2'>
			...
			/>
			<span className='text-fusionforge-fg-3 text-xs pointer-events-none'>{isReasoningEnabled ? `${currentEffortCapitalized}` : 'Thinking disabled'}</span>
		</div>
	}
...
interface FusionForgeChatAreaProps {
	// Required
...
export const FusionForgeChatArea: React.FC<FusionForgeChatAreaProps> = ({
	children,
	onSubmit,
	onAbort,
	onClose,
	...
	return (
		<div
			...
			className={`
				...
				border border-fusionforge-border-3 focus-within:border-fusionforge-border-1 hover:border-fusionforge-border-1
			...
			<div className="relative w-full">
				...
				{onClose && (
					<div className='absolute -top-1 -right-1 cursor-pointer z-1'>
						<IconX
							size={12}
							className="stroke-[2] opacity-80 text-fusionforge-fg-3 hover:brightness-95"
							onClick={onClose}
						/>
					</div>
				)}
				...
				{showModelDropdown && (
					<div className='flex flex-col gap-y-1'>
						...
						<div className='flex items-center flex-wrap gap-x-2 gap-y-1 text-nowrap '>
							{featureName === 'Chat' && <ChatModeDropdown className='text-xs text-fusionforge-fg-3 bg-fusionforge-bg-1 border border-fusionforge-border-2 rounded py-0.5 px-1' />}
							<ModelDropdown featureName={featureName} className='text-xs text-fusionforge-fg-3 bg-fusionforge-bg-1 rounded' />
						</div>
					</div>
				)}
...
export const SelectedFiles = (
	...
	const accessor = useAccessor()
	const commandService = accessor.get('ICommandService')
	const modelReferenceService = accessor.get('IFusionForgeModelService')
					...
					>
						...
						<div
							className={`
								...
								text-xs text-nowrap
								border rounded-sm
								${isThisSelectionProspective ? 'bg-fusionforge-bg-1 text-fusionforge-fg-3 opacity-80' : 'bg-fusionforge-bg-1 hover:brightness-95 text-fusionforge-fg-1'}
								${isThisSelectionProspective
									? 'border-fusionforge-border-2'
									: 'border-fusionforge-border-1'
								}
								hover:border-fusionforge-border-1
						...
						>
							...
							{selection.type === 'File' && selection.state.wasAddedAsCurrentFile && messageIdx === undefined && currentURI?.fsPath === selection.uri.fsPath ?
								<span className={`text-[8px] 'fusionforge-opacity-60 text-fusionforge-fg-4`}>
									{`(Current File)`}
								</span>
								: null
							}
...
const ToolHeaderWrapper = ({
	...
	const desc1HTML = <span
		className={`text-fusionforge-fg-4 text-xs italic truncate ml-2
	...
	return (<div className=''>
		<div className={`w-full border border-fusionforge-border-3 rounded px-2 py-1 bg-fusionforge-bg-3 overflow-hidden ${className}`}>
			{/* header */}
			<div className={`select-none flex items-center min-h-[24px]`}>
				<div className={`flex items-center w-full gap-x-2 overflow-hidden justify-between ${isRejected ? 'line-through' : ''}`}>
					...
					>
						...
						>
							{isDropdown && (<ChevronRight
								className={`
								text-fusionforge-fg-3 mr-0.5 h-4 w-4 flex-shrink-0 transition-transform duration-100 ease-[cubic-bezier(0.4,0,0.2,1)]
								${isExpanded ? 'rotate-90' : ''}
							`}
							/>)}
							<span className="text-fusionforge-fg-3 flex-shrink-0">{title}</span>
					...
					<div className="flex items-center gap-x-2 flex-shrink-0">
						...
						{info && <CircleEllipsis
							className='ml-2 text-fusionforge-fg-4 opacity-60 flex-shrink-0'
							size={14}
							data-tooltip-id='fusionforge-tooltip'
						...
						{isError && <AlertTriangle
							className='text-fusionforge-warning opacity-90 flex-shrink-0'
							size={14}
							data-tooltip-id='fusionforge-tooltip'
						...
						/>}
						{isRejected && <Ban
							className='text-fusionforge-fg-4 opacity-90 flex-shrink-0'
							size={14}
							data-tooltip-id='fusionforge-tooltip'
							data-tooltip-content={'Canceled'}
							data-tooltip-place='top'
						/>}
						{desc2 && <span className="text-fusionforge-fg-4 text-xs" onClick={desc2OnClick}>
							{desc2}
						</span>}
						{numResults !== undefined && (
							<span className="text-fusionforge-fg-4 text-xs ml-auto mr-1">
			...
			{<div
				className={`overflow-hidden transition-all duration-200 ease-in-out ${isExpanded ? 'opacity-100 py-1' : 'max-h-0 opacity-0'}
					text-fusionforge-fg-4 rounded-sm overflow-x-auto
				  `}
			//    bg-black bg-opacity-10 border border-fusionforge-border-4 border-opacity-50
			>
				{children}
			</div>}
		</div>
...
const EditTool = ({ toolMessage, threadId, messageIdx, content }: Parameters<ResultWrapper<'edit_file' | 'rewrite_file'>>[0] & { content: string }) => {
	...
	if (toolMessage.type === 'running_now' || toolMessage.type === 'tool_request') {
		componentParams.children = <ToolChildrenWrapper className='bg-fusionforge-bg-3'>
			<EditToolChildren
				uri={params.uri}
				code={content}
				type={editToolType}
		...
		<div>
			<div className="w-full">
				...
				>
					{isDropdown && (
						<ChevronRight
							className={`text-fusionforge-fg-3 mr-0.5 h-4 w-4 flex-shrink-0 transition-transform duration-100 ease-[cubic-bezier(0.4,0,0.2,1)] ${isOpen ? 'rotate-90' : ''}`}
						/>
					)}
					<div className="flex items-center w-full overflow-hidden">
						<span className="text-fusionforge-fg-3">{title}</span>
					</div>
				</div>
				{/* children */}
				{<div
					className={`overflow-hidden transition-all duration-200 ease-in-out ${isOpen ? 'opacity-100' : 'max-h-0 opacity-0'} text-fusionforge-fg-4`}
				>
					{children}
				</div>}
			</div>
...
const UserMessageComponent = ({ chatMessage, messageIdx, isCheckpointGhost, currCheckpointIdx, _scrollToBottom }: { chatMessage: ChatMessage & { role: 'user' }, messageIdx: number, currCheckpointIdx: number | undefined, isCheckpointGhost: boolean, _scrollToBottom: (() => void) | null }) => {
	...
	if (mode === 'display') {
		...
		chatbubbleContents = <FusionForgeChatArea
			featureName='Chat'
			onSubmit={onSubmit}
			onAbort={onAbort}
			isStreaming={false}
	...
	>
		<div
            ...
            ${mode === 'edit' ? ''
					: mode === 'display' ? 'p-2 flex flex-col bg-fusionforge-bg-1 text-fusionforge-fg-1 overflow-x-auto cursor-pointer' : ''
				}
        `}
		...
		>
			<EditSymbol
				...
				className={`
                    cursor-pointer
                    p-[2px]
                    bg-fusionforge-bg-1 border border-fusionforge-border-1 rounded-md
...
prose-code:text-fusionforge-fg-3
...
export const ListableToolItem = ({ name, onClick, isSmall, className, showDot }: { name: React.ReactNode, onClick?: () => void, isSmall?: boolean, className?: string, showDot?: boolean }) => {
	...
	>
		...
		<div className={`${isSmall ? 'italic text-fusionforge-fg-4 flex items-center' : ''}`}>{name}</div>
	</div>
}
...
const EditToolChildren = ({ uri, code, type }: { uri: URI | undefined, code: string, type: 'diff' | 'rewrite' }) => {
	...
	const content = type === 'diff' ?
		<FusionForgeDiffEditor uri={uri} searchReplaceBlocks={code} />
...
const LintErrorChildren = ({ lintErrors }: { lintErrors: LintErrorItem[] }) => {
	return <div className="text-xs text-fusionforge-fg-4 opacity-80 border-l-2 border-fusionforge-warning px-2 py-0.5 flex flex-col gap-0.5 overflow-x-auto whitespace-nowrap">
...
const BottomChildren = ({ children, title }: { children: React.ReactNode, title: string }) => {
	...
	return (
		<div className="w-full px-2 mt-0.5">
			...
			>
				<ChevronRight
					className={`mr-1 h-3 w-3 flex-shrink-0 transition-transform duration-100 text-fusionforge-fg-4 group-hover:text-fusionforge-fg-3 ${isOpen ? 'rotate-90' : ''}`}
				/>
				<span className="font-medium text-fusionforge-fg-4 group-hover:text-fusionforge-fg-3 text-xs">{title}</span>
			</div>
			<div
				className={`overflow-hidden transition-all duration-200 ease-in-out ${isOpen ? 'opacity-100' : 'max-h-0 opacity-0'} text-xs pl-4`}
			>
				<div className="overflow-x-auto text-fusionforge-fg-4 opacity-90 border-l-2 border-fusionforge-warning px-2 py-0.5">
					{children}
				</div>
			</div>
		</div>
...
const InvalidTool = ({ toolName, message, mcpServerName }: { toolName: ToolName, message: string, mcpServerName: string | undefined }) => {
	...
	componentParams.children = <ToolChildrenWrapper>
		<CodeChildren className='bg-fusionforge-bg-3'>
			{message}
		</CodeChildren>
	</ToolChildrenWrapper>
	return <ToolHeaderWrapper {...componentParams} />
	...
	const fileDetailsContent = <div className="px-2 gap-1 w-full overflow-y-auto">
		{sortedCommandBarURIs.map((uri, i) => {
			...
			const fileNameHTML = <div
				className="flex items-center gap-1.5 text-fusionforge-fg-3 hover:brightness-125 transition-all duration-200 cursor-pointer"
				onClick={() => voidOpenFileFn(uri, accessor)}
			>
				{/* <FileIcon size={14} className="text-fusionforge-fg-3" /> */}
				<span className="text-fusionforge-fg-3">{basename}</span>
			</div>
			...
			const detailsContent = <div className='flex px-4'>
				<span className="text-fusionforge-fg-3 opacity-80">{numDiffs} diff{numDiffs !== 1 ? 's' : ''}</span>
			</div>
		...
		<>
			...
			<div className='px-2'>
				<div
					className={`
						select-none
						flex w-full rounded-t-lg bg-fusionforge-bg-3
						text-fusionforge-fg-3 text-xs text-nowrap
...
export const SidebarChat = () => {
	...
	const inputChatArea = <FusionForgeChatArea
		featureName='Chat'
		onSubmit={() => onSubmit()}
		onAbort={onAbort}
	...
	const initiallySuggestedPromptsHTML = <div className='flex flex-col gap-2 w-full text-nowrap text-fusionforge-fg-3 select-none'>
		{[
	...
	>
		...
		{Object.keys(chatThreadsState.allThreads).length > 1 ? // show if there are threads
			<ErrorBoundary>
				<div className='pt-8 mb-2 text-fusionforge-fg-3 text-root select-none pointer-events-none'>Previous Threads</div>
				<PastThreadsList />
			</ErrorBoundary>
			:
			<ErrorBoundary>
				<div className='pt-8 mb-2 text-fusionforge-fg-3 text-root select-none pointer-events-none'>Suggestions</div>
