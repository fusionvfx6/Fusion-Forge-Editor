...
export const FusionForgeInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
	...
	return <>
		<textarea
			...
			className={`w-full resize-none max-h-[500px] overflow-y-auto text-fusionforge-fg-1 placeholder:text-fusionforge-fg-3 ${className}`}
			style={{
		...
		{isMenuOpen && (
			<div
				...
				className="z-[100] border-fusionforge-border-3 bg-fusionforge-bg-2-alt border rounded shadow-lg flex flex-col overflow-hidden"
				style={{
					position: strategy,
					top: y ?? 0,
					left: x ?? 0,
			...
			>
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="px-2 py-1 text-fusionforge-fg-1 bg-fusionforge-bg-2-alt border-b border-fusionforge-border-3 sticky top-0 bg-fusionforge-bg-1 z-10 select-none pointer-events-none">
					{optionText ?
				...
				<div className='max-h-[400px] w-full max-w-full overflow-y-auto overflow-x-auto'>
					<div className="w-max min-w-full flex flex-col gap-0 text-nowrap flex-nowrap">
						{options.length === 0 ?
							<div className="text-fusionforge-fg-3 px-3 py-0.5">No results found</div>
							: options.map((o, oIdx) => {
								...
								return (
									...
									<div
										...
										className={`
											...
											${oIdx === optionIdx ? 'bg-blue-500 text-white/80' : 'bg-fusionforge-bg-2-alt text-fusionforge-fg-1'}
										`}
...
export const FusionForgeSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps }: {
	value: string;
	...
	return (
		<input
			...
			placeholder={placeholder}
			disabled={disabled}
			className={`w-full resize-none bg-fusionforge-bg-1 text-fusionforge-fg-1 placeholder:text-fusionforge-fg-3 border border-fusionforge-border-2 focus:border-fusionforge-border-1
...
export const FusionForgeInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline }: {
	onChangeText: (value: string) => void;
	...
	return <WidgetComponent
		className='
			bg-fusionforge-bg-1
			@@fusionforge-force-child-placeholder-fusionforge-fg-1
		'
		ctor={InputBox}
...
export const FusionForgeSlider = ({
	value,
	onChange,
	size = 'md',
	disabled = false,
				...
				<div className="relative w-full">
					...
					<div
						className={`relative ${size === 'xxs' ? 'h-0.5' :
							size === 'xs' ? 'h-1' :
								size === 'sm' ? 'h-1.5' :
									size === 'sm+' ? 'h-2' : 'h-2.5'
							} bg-fusionforge-bg-2 rounded-full cursor-pointer`}
						onClick={handleTrackClick}
					>
						...
						<div
							className={`absolute left-0 ${size === 'xxs' ? 'h-0.5' :
								size === 'xs' ? 'h-1' :
									size === 'sm' ? 'h-1.5' :
										size === 'sm+' ? 'h-2' : 'h-2.5'
								} bg-fusionforge-fg-1 rounded-full`}
					...
					<div
						className={`absolute top-1/2 transform -translate-x-1/2 -translate-y-1/2
							...
							bg-fusionforge-fg-1 rounded-full shadow-md ${disabled ? 'cursor-not-allowed' : 'cursor-grab active:cursor-grabbing'}
							border border-fusionforge-fg-1`}
...
export const FusionForgeSwitch = ({
	value,
	onChange,
	size = 'md',
	disabled = false,
...
export const FusionForgeCheckBox = ({ label, value, onClick, className }: { label: string, value: boolean, onClick: (checked: boolean) => void, className?: string }) => {
...
export const FusionForgeCustomDropdownBox = <T extends NonNullable<any>>({
	options,
	selectedOption,
	onChangeOption,
	...
	return (
		<div className={`inline-block relative ${className}`}>
			...
			{isOpen && (
				<div
					ref={refs.setFloating}
					className="z-[100] bg-fusionforge-bg-1 border-fusionforge-border-3 border rounded shadow-lg"
					style={{
						position: strategy,
						top: y ?? 0,
						left: x ?? 0,
...
export const _FusionForgeSelectBox = <T,>({ onChangeSelection, onCreateInstance, selectBoxRef, options, className }: {
	...
	return <WidgetComponent
		className={`
			...
			@@[&_select]:!fusionforge-text-fusionforge-fg-3
			@@[&_select]:!fusionforge-text-xs
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
	...
	return <div ref={divRef} className='relative z-0 px-2 py-1 bg-fusionforge-bg-3'>
		<WidgetComponent
...
export const FusionForgeButtonBgDarken = ({ children, disabled, onClick, className }: { children: React.ReactNode; disabled?: boolean; onClick: () => void; className?: string }) => {
...
// export const FusionForgeScrollableElt = ({ options, children }: { options: ScrollableElementCreationOptions, children: React.ReactNode }) => {
...
// export const FusionForgeSelectBox = <T,>({ onChangeSelection, initVal, selectBoxRef, options }: {
// 	initVal: T;
// 	selectBoxRef: React.MutableRefObject<SelectBox | null>;
// 	options: readonly { text: string, value: T }[];
// 	onChangeSelection: (value: T) => void;
...
// export const FusionForgeCheckBox = ({ onChangeChecked, initVal, label, checkboxRef, }: {
...
const SingleDiffEditor = ({ block, lang }: { block: ExtractedSearchReplaceBlock, lang: string | undefined }) => {
	...
	return (
		<div className="w-full bg-fusionforge-bg-3 @@bg-editor-style-override" ref={divRef} />
	);
};
 ...
 */
export const FusionForgeDiffEditor = ({ uri, searchReplaceBlocks, language }: { uri?: any, searchReplaceBlocks: string, language?: string }) => {
	const accessor = useAccessor();
	const languageService = accessor.get('ILanguageService');
	...
	if (blocks.length === 0) {
		return <div className="w-full p-4 text-fusionforge-fg-4 text-sm">No changes found</div>;
	}
		...
		<div className="w-full flex flex-col gap-2">
			{blocks.map((block, index) => (
				<div key={index} className="w-full">
					{blocks.length > 1 && (
						<div className="text-fusionforge-fg-4 text-xs mb-1 px-1">
