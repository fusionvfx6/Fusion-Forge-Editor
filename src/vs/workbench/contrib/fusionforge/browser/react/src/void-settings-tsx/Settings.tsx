...
import { ProviderName, SettingName, displayInfoOfSettingName, providerNames, FusionForgeStatefulModelInfo, customSettingNamesOfProvider, RefreshableProviderName, refreshableProviderNames, displayInfoOfProviderName, nonlocalProviderNames, localProviderNames, GlobalSettingName, featureNames, displayInfoOfFeatureName, isProviderNameDisabled, FeatureName, hasDownloadButtonsOnModelsProviderNames, subTextMdOfProviderName } from '../../../../common/voidSettingsTypes.js'
import ErrorBoundary from '../sidebar-tsx/ErrorBoundary.js'
import { FusionForgeButtonBgDarken, FusionForgeCustomDropdownBox, FusionForgeInputBox2, FusionForgeSimpleInputBox, FusionForgeSwitch } from '../util/inputs.js'
...
const ButtonLeftTextRightOption = ({ text, leftButton }: { text: string, leftButton?: React.ReactNode }) => {
	...
	return <div className='flex items-center text-fusionforge-fg-3 px-3 py-0.5 rounded-sm overflow-hidden gap-2'>
...
const ConfirmButton = ({ children, onConfirm, className }: { children: React.ReactNode, onConfirm: () => void, className?: string }) => {
	...
	return (
		<div ref={ref} className={`inline-block`}>
			<FusionForgeButtonBgDarken className={className} onClick={() => {
				if (!confirm) {
					setConfirm(true);
				} else {
					onConfirm();
...
const SimpleModelSettingsDialog = ({
	...
	const settingsStateService = accessor.get('IFusionForgeSettingsService')
	...
	const sourcecodeOverridesLink = `https://github.com/fusionvfx6/Fusion-Forge-Editor/blob/main/src/vs/workbench/contrib/fusionforge/common/modelCapabilities.ts#L146-L172`
	...
	return (
		...
		>
			{/* MODAL */}
			<div
				className="bg-fusionforge-bg-1 rounded-md p-4 max-w-xl w-full shadow-xl overflow-y-auto max-h-[90vh]"
			...
			>
				<div className="flex justify-between items-center mb-4">
					...
					<button
						onClick={onClose}
						className="text-fusionforge-fg-3 hover:text-fusionforge-fg-1"
					>
						<X className="size-5" />
					</button>
				</div>
				...
				<div className="text-sm text-fusionforge-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with FusionForge, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by FusionForge.`
							: `FusionForge recognizes ${modelName} ("${recognizedModelName}").`}
				</div>


				{/* override toggle */}
				<div className="flex items-center gap-2 mb-4">
					<FusionForgeSwitch size='xs' value={overrideEnabled} onChange={setOverrideEnabled} />
					<span className="text-fusionforge-fg-3 text-sm">Override model defaults</span>
				</div>

				{/* Informational link */}
				{overrideEnabled && <div className="text-sm text-fusionforge-fg-3 mb-4">
				...
				<textarea
					key={overrideEnabled + ''}
					ref={textAreaRef}
					className={`w-full min-h-[200px] p-2 rounded-sm border border-fusionforge-border-2 bg-fusionforge-bg-2 resize-none font-mono text-sm ${!overrideEnabled ? 'text-fusionforge-fg-3' : ''}`}
				...
				<div className="flex justify-end gap-2 mt-4">
					<FusionForgeButtonBgDarken onClick={onClose} className="px-3 py-1">
						Cancel
					</FusionForgeButtonBgDarken>
					<FusionForgeButtonBgDarken
						onClick={onSave}
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	const modelDump: (FusionForgeStatefulModelInfo & { providerName: ProviderName, providerEnabled: boolean })[] = []
	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='fusionforge-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='fusionforge-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
				...
				<div className="flex items-center gap-2 w-fit">
					...
					<div className={`w-5 flex items-center justify-center`}>
						{type === 'default' || type === 'autodetected' ? null : <button
							...
							className={`${hasOverrides ? '' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}
						>
							<X size={12} className="text-fusionforge-fg-3 opacity-50" />
						</button>}
					</div>
				</div>
			</div>
		...
		{showCheckmark ? (
				...
				<form className="flex items-center gap-2">
					...
					<ErrorBoundary>
						<FusionForgeCustomDropdownBox
							...
							className="max-w-32 mx-2 w-full resize-none bg-fusionforge-bg-1 text-fusionforge-fg-1 placeholder:text-fusionforge-fg-3 border border-fusionforge-border-2 focus:border-fusionforge-border-1 py-1 px-2 rounded"
					...
					<button
						...
						className='text-fusionforge-fg-4'
					>
						<X className='size-4' />
					</button>
				</form>
			...
			<div
				className="text-fusionforge-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer mt-4"
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	const accessor = useAccessor()
	const voidSettingsService = accessor.get('IFusionForgeSettingsService')
	const settingsState = useSettingsState()
...
export const FusionForgeProviderSettings = ({ providerNames }: { providerNames: ProviderName[] }) => {
	return <>
...
const FastApplyMethodDropdown = () => {
	...
	return <FusionForgeCustomDropdownBox
		className='text-xs text-fusionforge-fg-3 bg-fusionforge-bg-1 border border-fusionforge-border-1 rounded p-0.5 px-1'
		options={options}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-fusionforge-fg-3 text-sm list-decimal select-text'>
		<div className=''><ChatMarkdownRender string={`Ollama Setup Instructions`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`1. Download [Ollama](https://ollama.com/download).`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`2. Open your terminal.`} chatMessageLocation={undefined} /></div>
		<div
			className='pl-6 flex items-center w-fit'
			data-tooltip-id='fusionforge-tooltip-ollama-settings'
		>
			<ChatMarkdownRender string={`3. Run \`ollama pull your_model\` to install a model.`} chatMessageLocation={undefined} />
		</div>
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`FusionForge automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	const voidSettingsService = accessor.get('IFusionForgeSettingsService')
	return <div
		className={`text-fusionforge-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer ${className}`}
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	return <>
		...
		/>
		<span className="text-fusionforge-fg-3 text-xs">{desc}</span>
	</>
}
...
export const OneClickSwitchButton = ({ fromEditor = 'VS Code', className = '' }: { fromEditor?: TransferEditorType, className?: string }) => {
	...
	return <>
		<FusionForgeButtonBgDarken className={`max-w-48 p-4 ${className}`} disabled={transferState.type !== 'done'} onClick={onClick}>
...
const MCPServerComponent = ({ name, server }: { name: string, server: MCPServer }) => {
	...
	return (
		<div className="border border-fusionforge-border-2 bg-fusionforge-bg-1 py-3 px-4 rounded-sm my-2">
						...
						${server.status === 'success' ? 'bg-green-500'
							: server.status === 'error' ? 'bg-red-500'
								: server.status === 'loading' ? 'bg-yellow-500'
									: server.status === 'offline' ? 'bg-fusionforge-fg-3'
										: ''}
					`}></div>

					{/* Server name */}
					<div className="text-sm font-medium text-fusionforge-fg-1">{name}</div>
				</div>
					...
					<div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
						{(server.tools ?? []).length > 0 ? (
							(server.tools ?? []).map((tool: { name: string; description?: string }) => (
								<span
									key={tool.name}
									className="px-2 py-0.5 bg-fusionforge-bg-2 text-fusionforge-fg-3 rounded-sm text-xs"

									data-tooltip-id='fusionforge-tooltip'
									data-tooltip-content={tool.description || ''}
									data-tooltip-class-name='fusionforge-max-w-[300px]'
								>
									{removeUniquePrefix(tool.name)}
								</span>
							))
						) : (
							<span className="text-xs text-fusionforge-fg-3">No tools available</span>
						)}
					</div>
				</div>
			)}
			...
			{isOn && server.command && (
				<div className="mt-3">
					<div className="text-xs text-fusionforge-fg-3 mb-1">Command:</div>
					<div className="px-2 py-1 bg-fusionforge-bg-2 text-xs font-mono overflow-x-auto whitespace-nowrap text-fusionforge-fg-2 rounded-sm">
						{server.command}
					</div>
				</div>
			)}
...
const MCPServersList = () => {
	...
	if (mcpServiceState.error) {
		content = <div className="text-fusionforge-fg-3 text-sm mt-2">
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			downloadName = 'fusionforge-chats.json'
		}
		else if (t === 'Settings') {
			// Export user settings
			dataStr = JSON.stringify(voidSettingsService.state, null, 2)
			downloadName = 'fusionforge-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
	...
	return (
		<div className={`@@fusionforge-scope ${isDark ? 'dark' : ''}`} style={{ height: '100%', width: '100%', overflow: 'auto' }}>
          ...
          ${selectedSection === tab
										...
										: 'bg-fusionforge-bg-2 hover:bg-fusionforge-bg-2/80 text-fusionforge-fg-1'}
        `}
							>
								{label}
							</button>
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`FusionForge's Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-fusionforge-fg-3 mb-2`}>{`FusionForge can access any model that you host locally. We automatically detect your local models by default.`}</h3>

									<div className='opacity-80 mb-4'>
										<OllamaSetupInstructions sayWeAutoDetect={true} />
									</div>

									<FusionForgeProviderSettings providerNames={localProviderNames} />
								</ErrorBoundary>
							</div>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-fusionforge-fg-3 mb-2`}>{`FusionForge can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>

									<FusionForgeProviderSettings providerNames={nonlocalProviderNames} />
								</ErrorBoundary>
							</div>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										<ErrorBoundary>
											...
											<div>
												...
												<div className='text-sm text-fusionforge-fg-3 mt-1'>
													<span>
														Experimental.{' '}
													</span>
													<span
														...
														data-tooltip-class-name='fusionforge-max-w-[20px]'
													>
												...
												<div className='my-2'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															...
															/>
															<span className='text-fusionforge-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.enableAutocomplete ? 'Enabled' : 'Disabled'}</span>
														</div>
													</ErrorBoundary>
													...
													<ErrorBoundary>
														<div className={`my-2 ${!settingsState.globalSettings.enableAutocomplete ? 'hidden' : ''}`}>
															<ModelDropdown featureName={'Autocomplete'} className='text-xs text-fusionforge-fg-3 bg-fusionforge-bg-1 border border-fusionforge-border-1 rounded p-0.5 px-1' />
														</div>
													</ErrorBoundary>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-fusionforge-fg-3 mt-1'>Settings that control the behavior of the Apply button.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														...
														/>
														<span className='text-fusionforge-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncApplyToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncApplyToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'Apply'} className='text-xs text-fusionforge-fg-3 bg-fusionforge-bg-1 border border-fusionforge-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
										...
										<div>
											...
											<div className='text-sm text-fusionforge-fg-3 mt-1'>{`Tools are functions that LLMs can call. Some tools require user approval.`}</div>
											...
											<div className='my-2'>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														...
														/>
														<span className='text-fusionforge-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.includeToolLintErrors ? 'Fix lint errors' : `Fix lint errors`}</span>
													</div>
												</ErrorBoundary>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														...
														/>
														<span className='text-fusionforge-fg-3 text-xs pointer-events-none'>Auto-accept LLM changes</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-fusionforge-fg-3 mt-1'>{`Settings that control the visibility of FusionForge suggestions in the code editor.`}</div>
											...
											<div className='my-2'>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														...
														/>
														<span className='text-fusionforge-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.showInlineSuggestions ? 'Show suggestions on select' : 'Show suggestions on select'}</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-fusionforge-fg-3 mt-1'>Settings that control the behavior of the commit message generator.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														...
														/>
														<span className='text-fusionforge-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncSCMToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncSCMToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'SCM'} className='text-xs text-fusionforge-fg-3 bg-fusionforge-bg-1 border border-fusionforge-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-fusionforge-fg-3 mb-4'>{`Transfer your editor settings into FusionForge.`}</h4>
								...
								<div>
									...
									<h4 className='text-fusionforge-fg-3 mb-4'>{`Transfer FusionForge's settings and chats in and out of FusionForge.`}</h4>
									<div className='flex flex-col gap-8'>
										{/* Settings Subcategory */}
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											<input key={2 * s} ref={fileInputSettingsRef} type='file' accept='.json' className='hidden' onChange={handleUpload('Settings')} />
											<FusionForgeButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputSettingsRef.current?.click() }}>
												Import Settings
											</FusionForgeButtonBgDarken>
											<FusionForgeButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Settings')}>
												Export Settings
											</FusionForgeButtonBgDarken>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<FusionForgeButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputChatsRef.current?.click() }}>
												Import Chats
											</FusionForgeButtonBgDarken>
											<FusionForgeButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Chats')}>
												Export Chats
											</FusionForgeButtonBgDarken>
								...
								<div>
									...
									<h4 className={`text-fusionforge-fg-3 mb-4`}>{`IDE settings, keyboard settings, and theme customization.`}</h4>

									<ErrorBoundary>
										<div className='flex flex-col gap-2 justify-center max-w-48 w-full'>
											<FusionForgeButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openSettings') }}>
												General Settings
											</FusionForgeButtonBgDarken>
											<FusionForgeButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openGlobalKeybindings') }}>
												Keyboard Settings
											</FusionForgeButtonBgDarken>
											<FusionForgeButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.selectTheme') }}>
												Theme Settings
											</FusionForgeButtonBgDarken>
											<FusionForgeButtonBgDarken className='px-4 py-1' onClick={() => { nativeHostService.showItemInFolder(environmentService.logsHome.fsPath) }}>
												Open Logs
											</FusionForgeButtonBgDarken>
										</div>
									</ErrorBoundary>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-fusionforge-fg-3 mb-4`}>Very basic anonymous usage tracking helps us keep FusionForge running smoothly. You may opt out below. Regardless of this setting, FusionForge never sees your code, messages, or API keys.</h4>
									...
									<div className='my-2'>
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2 my-2'>
												...
												/>
												<span className='text-fusionforge-fg-3 text-xs pointer-events-none'>{'Opt-out (requires restart)'}</span>
											</div>
										</ErrorBoundary>
									</div>
								</div>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-fusionforge-fg-3 mb-4`}>
									...
									<div className='my-4'>
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												...
												/>
												<span className='text-fusionforge-fg-3 text-xs pointer-events-none'>
													{'Disable system message'}
												</span>
											</div>
										</ErrorBoundary>
										<div className='text-fusionforge-fg-3 text-xs mt-1'>
											{`When disabled, FusionForge will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>
...
Use Model Context Protocol to provide Agent mode with more tools.
							`} chatMessageLocation={undefined} />
									...
									<div className='my-2'>
										<FusionForgeButtonBgDarken className='px-4 py-1 w-full max-w-48' onClick={async () => { await mcpService.revealMCPConfigFile() }}>
											Add MCP Server
										</FusionForgeButtonBgDarken>
									</div>
