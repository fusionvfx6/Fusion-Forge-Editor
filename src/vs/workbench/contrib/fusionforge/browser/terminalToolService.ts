...
export const persistentTerminalNameOfId = (id: string) => {
	if (id === '1') return 'FusionForge Agent'
	return `FusionForge Agent (${id})`
}
export const idOfPersistentTerminalName = (name: string) => {
	if (name === 'FusionForge Agent') return '1'

	const match = name.match(/FusionForge Agent \((\d+)\)/)
...
export class TerminalToolService extends Disposable implements ITerminalToolService {
	...
	runCommand: ITerminalToolService['runCommand'] = async (command, params) => {
		...
		const waitForResult = async () => {
			...
			// if (!cmdCap) throw new Error(`There was an error using the terminal: CommandDetection capability did not mount yet. Please try again in a few seconds or report this to the FusionForge team.`)
