...
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, FusionForgeStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './voidSettingsTypes.js';
...
export type FusionForgeSettingsState = {
...
// type RealFusionForgeSettings = Exclude<keyof FusionForgeSettingsState, '_modelOptions'>
// type EventProp<T extends RealFusionForgeSettings = RealFusionForgeSettings> = T extends 'globalSettings' ? [T, keyof FusionForgeSettingsState[T]] : T | 'all'


export interface IFusionForgeSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: FusionForgeSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: FusionForgeSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: FusionForgeStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, FusionForgeStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: FusionForgeSettingsState): FusionForgeSettingsState => {
...
const _validatedModelState = (state: Omit<FusionForgeSettingsState, '_modelOptions'>): FusionForgeSettingsState => {
	...
	} satisfies FusionForgeSettingsState
...
const defaultState = () => {
	const d: FusionForgeSettingsState = {
...
export const IFusionForgeSettingsService = createDecorator<IFusionForgeSettingsService>('FusionForgeSettingsService');
class FusionForgeSettingsService extends Disposable implements IFusionForgeSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: FusionForgeSettingsState;
	...
	dangerousSetState = async (newState: FusionForgeSettingsState) => {
	...
	async readAndInitializeState() {
		let readS: FusionForgeSettingsState
		try {
	...
	private async _readState(): Promise<FusionForgeSettingsState> {
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: FusionForgeSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: FusionForgeStatefulModelInfo[] = [
...
registerSingleton(IFusionForgeSettingsService, FusionForgeSettingsService, InstantiationType.Eager);
