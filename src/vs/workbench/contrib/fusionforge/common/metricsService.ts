...
export class MetricsService implements IMetricsService {
	...
	constructor(
		...
		this.metricsService = ProxyChannel.toService<IMetricsService>(mainProcessService.getChannel('fusionforge-channel-metrics'));
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'voidDebugInfo',
			f1: true,
			title: localize2('voidMetricsDebug', 'FusionForge: Log Debug Info'),
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		notifService.info(`FusionForge Debug info:\n${JSON.stringify(debugProperties, null, 2)}`)
	}
})
