...
export type FusionForgeStaticModelInfo = { // not stateful
	// FusionForge uses the information below to know how to handle each model.
...
type FusionForgeStaticProviderInfo = { // doesn't change (not stateful)
	...
	modelOptions: { [key: string]: FusionForgeStaticModelInfo };
	modelOptionsFallback: (modelName: string, fallbackKnownValues?: Partial<FusionForgeStaticModelInfo>) => (FusionForgeStaticModelInfo & { modelName: string, recognizedModelName: string }) | null;
}
...
const defaultModelOptions = {
	...
	supportsFIM: false,
	reasoningCapabilities: false,
} as const satisfies FusionForgeStaticModelInfo
...
} as const satisfies { [s: string]: Partial<FusionForgeStaticModelInfo> }




// keep modelName, but use the fallback's defaults
const extensiveModelOptionsFallback: FusionForgeStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {

	const lower = modelName.toLowerCase()

	const toFallback = <T extends { [s: string]: Omit<FusionForgeStaticModelInfo, 'cost' | 'downloadable'> },>(obj: T, recognizedModelName: string & keyof T)
		: FusionForgeStaticModelInfo & { modelName: string, recognizedModelName: string } => {
	...
	// Heurística de último recurso para modelos de código
	if (lower.includes('codellama') || lower.includes('deepseek-coder') || lower.includes('coder') || lower.includes('code') || lower.includes('fim')) {
		return toFallback(openSourceModelOptions_assumingOAICompat, 'qwen2.5coder') // Usamos un template genérico con supportsFIM: true
	}
...
} as const satisfies { [s: string]: FusionForgeStaticModelInfo }

const anthropicSettings: FusionForgeStaticProviderInfo = {
...
const openAISettings: FusionForgeStaticProviderInfo = {
...
const xAISettings: FusionForgeStaticProviderInfo = {
...
const geminiSettings: FusionForgeStaticProviderInfo = {
...
const deepseekSettings: FusionForgeStaticProviderInfo = {
...
const mistralSettings: FusionForgeStaticProviderInfo = {
...
const groqSettings: FusionForgeStaticProviderInfo = {
...
} as const satisfies Record<string, FusionForgeStaticModelInfo>
const googleVertexSettings: FusionForgeStaticProviderInfo = {
...
const microsoftAzureSettings: FusionForgeStaticProviderInfo = {
...
const awsBedrockSettings: FusionForgeStaticProviderInfo = {
...
const vLLMSettings: FusionForgeStaticProviderInfo = {
...
const lmStudioSettings: FusionForgeStaticProviderInfo = {
...
const ollamaSettings: FusionForgeStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
...
const openaiCompatible: FusionForgeStaticProviderInfo = {
...
const liteLLMSettings: FusionForgeStaticProviderInfo = { // https://docs.litellm.ai/docs/reasoning_content
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
	modelOptions: {},
	providerReasoningIOSettings: {
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const openRouterSettings: FusionForgeStaticProviderInfo = {
...
const modelSettingsOfProvider: { [providerName in ProviderName]: FusionForgeStaticProviderInfo } = {
