...
export class FusionForgeMainUpdateService extends Disposable implements IFusionForgeUpdateService {
	...
	async check(explicit: boolean): Promise<FusionForgeCheckUpdateResponse> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart FusionForge to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<FusionForgeCheckUpdateResponse> {
		try {
			const response = await fetch('https://api.github.com/repos/fusionvfx6/Fusion-Forge-Editor/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of FusionForge is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'FusionForge is up-to-date!'
					}
				}
				else {
