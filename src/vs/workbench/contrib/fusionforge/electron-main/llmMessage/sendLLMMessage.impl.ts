...
const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any } }) => {
	...
	if (providerName === 'openAI') {
		...
		return new OpenAI({
			...
			defaultHeaders: {
				'HTTP-Referer': 'https://github.com/fusionvfx6/Fusion-Forge-Editor', // Optional, for including your app on openrouter.ai rankings.
				'X-Title': 'Fusion Forge', // Optional. Shows in rankings on openrouter.ai.
			},
			...commonPayloadOpts,
		})
	}
	...
	else throw new Error(`FusionForge providerName was invalid: ${providerName}.`)
}
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				onError({ message: 'FusionForge: Response from model was empty.', fullError: null })
			}
			else {
...
const newOllamaSDK = ({ endpoint }: { endpoint: string }) => {
	...
	if (!endpoint) throw new Error(`Ollama Endpoint was empty (please enter ${defaultProviderSettings.ollama.endpoint} in FusionForge if you want the default url).`)
...
const sendOllamaFIM = ({ messages, onText, onFinalMessage, onError, settingsOfProvider, modelName, _setAborter, providerName, overridesOfModel }: SendFIMParams_Internal) => {
	...
	if (!supportsFIM) {
		// FALLBACK FIM -> Chat
		const systemPrompt = `You are a strict code completion engine. Your task is to complete the code exactly where the cursor is.
Output ONLY the raw code to be inserted. Do NOT output any conversational text, explanations, or markdown code blocks (do not use \`\`\`). Just the raw code.`
