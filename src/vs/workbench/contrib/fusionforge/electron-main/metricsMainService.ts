...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/FusionForge
	...
	private get oldId() {
		...
		const newKey = 'fusionforge.app.oldMachineId'
		...
		const oldValue = this._appStorage.get('fusionforge.machineId', StorageScope.APPLICATION) ?? 'NULL' // the old way of getting the key
		...
		// private get oldId() {
		// 	return this._memoStorage('fusionforge.app.oldMachineId', StorageTarget.MACHINE, 'NULL')
		// }
	}
	...
	private get distinctId() {
		...
		return this._memoStorage('fusionforge.app.machineId', StorageTarget.MACHINE, setValIfNotExist)
	}

	// just to see if there are ever multiple machineIDs per userID (instead of this, we should just track by the user's email)
	private get userId() {
		return this._memoStorage('fusionforge.app.userMachineId', StorageTarget.USER)
	}
	...
	async initialize() {
		...
		console.log('User is opted out of basic FusionForge metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('FusionForge posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
