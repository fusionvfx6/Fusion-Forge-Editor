...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	private _getCurrentVoidFileSnapshot = (uri: URI): FusionForgeFileSnapshot => {
		const { model } = this._voidModelService.getModel(uri)
	...
	private _restoreVoidFileSnapshot = async (uri: URI, snapshot: FusionForgeFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: FusionForgeFileSnapshot = this._getCurrentVoidFileSnapshot(uri)
	...
	public restoreVoidFileSnapshot(uri: URI, snapshot: FusionForgeFileSnapshot): void {
