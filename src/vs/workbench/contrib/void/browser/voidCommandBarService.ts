...
export class FusionForgeCommandBarService extends Disposable implements IVoidCommandBarService {
...
registerSingleton(IVoidCommandBarService, FusionForgeCommandBarService, InstantiationType.Delayed); // delayed is needed here :(
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountVoidCommandBar(root, accessor, { uri, editor } satisfies FusionForgeCommandBarProps)
			if (!res) return
